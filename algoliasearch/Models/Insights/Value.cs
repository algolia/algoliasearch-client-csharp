...
public class ValueJsonConverter : JsonConverter<Value>
{
  ...
  public override Value Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
  {
    ...
    if (root.ValueKind == JsonValueKind.Number)
    {
      try
      {
        return new Value(jsonDocument.Deserialize<double>(JsonConfig.Options));
      }
    ...
    if (root.ValueKind == JsonValueKind.String)
    {
      try
      {
        return new Value(jsonDocument.Deserialize<string>(JsonConfig.Options));
      }
      catch (Exception exception)
      {
        // deserialization failed, try the next one
  ...
  /// <param name="value">Value to be converted into a JSON string</param>
