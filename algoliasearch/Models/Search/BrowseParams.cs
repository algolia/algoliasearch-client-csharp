...
public partial class BrowseParams : AbstractSchema
{
  ...
  {
    if (obj is not BrowseParams input)
    {
      return false;
    }
...
/// </summary>
public class BrowseParamsJsonConverter : JsonConverter<BrowseParams>
{
  ...
  {
    return objectType == typeof(BrowseParams);
  }
  ...
  public override BrowseParams Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
  {
    var jsonDocument = JsonDocument.ParseValue(ref reader);
    var root = jsonDocument.RootElement;
    if (root.ValueKind == JsonValueKind.Object && root.TryGetProperty("params", out _))
    {
      try
      {
        return new BrowseParams(jsonDocument.Deserialize<SearchParamsString>(JsonConfig.Options));
      }
      catch (Exception exception)
      {
        // deserialization failed, try the next one
        System.Diagnostics.Debug.WriteLine($"Failed to deserialize into SearchParamsString: {exception}");
      }
    }
    if (root.ValueKind == JsonValueKind.Object)
    {
      try
      {
        return new BrowseParams(jsonDocument.Deserialize<BrowseParamsObject>(JsonConfig.Options));
      }
      catch (Exception exception)
      {
        // deserialization failed, try the next one
        System.Diagnostics.Debug.WriteLine($"Failed to deserialize into BrowseParamsObject: {exception}");
      }
    }
  ...
  /// <param name="value">BrowseParams to be converted into a JSON string</param>
  /// <param name="options">JSON Serializer options</param>
  public override void Write(Utf8JsonWriter writer, BrowseParams value, JsonSerializerOptions options)
  {
