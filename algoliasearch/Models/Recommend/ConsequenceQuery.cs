...
public partial class ConsequenceQuery : AbstractSchema
{
  ...
  {
    if (obj is not ConsequenceQuery input)
    {
      return false;
    }
...
/// </summary>
public class ConsequenceQueryJsonConverter : JsonConverter<ConsequenceQuery>
{
  ...
  {
    return objectType == typeof(ConsequenceQuery);
  }
  ...
  public override ConsequenceQuery Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
  {
    ...
    if (root.ValueKind == JsonValueKind.Object)
    {
      try
      {
        return new ConsequenceQuery(jsonDocument.Deserialize<ConsequenceQueryObject>(JsonConfig.Options));
      }
      catch (Exception exception)
      {
        // deserialization failed, try the next one
        System.Diagnostics.Debug.WriteLine($"Failed to deserialize into ConsequenceQueryObject: {exception}");
      }
    }
    if (root.ValueKind == JsonValueKind.String)
    {
      try
      {
        return new ConsequenceQuery(jsonDocument.Deserialize<string>(JsonConfig.Options));
      }
      catch (Exception exception)
      {
        // deserialization failed, try the next one
  ...
  /// <param name="value">ConsequenceQuery to be converted into a JSON string</param>
