...
public partial class TagFilters : AbstractSchema
{
  ...
  /// <summary>
  /// Get the actual instance of `List{TagFilters}`. If the actual instance is not `List{TagFilters}`,
  ...
  {
    if (obj is not TagFilters input)
    {
      return false;
    }
...
/// </summary>
public class TagFiltersJsonConverter : JsonConverter<TagFilters>
{
  ...
  {
    return objectType == typeof(TagFilters);
  }
  ...
  public override TagFilters Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
  {
    ...
    if (root.ValueKind == JsonValueKind.Array)
    {
      try
      {
        return new TagFilters(jsonDocument.Deserialize<List<TagFilters>>(JsonConfig.Options));
      }
      catch (Exception exception)
      {
        // deserialization failed, try the next one
        System.Diagnostics.Debug.WriteLine($"Failed to deserialize into List<TagFilters>: {exception}");
      }
    }
    if (root.ValueKind == JsonValueKind.String)
    {
      try
      {
        return new TagFilters(jsonDocument.Deserialize<string>(JsonConfig.Options));
      }
      catch (Exception exception)
      {
        // deserialization failed, try the next one
  ...
  /// <param name="value">TagFilters to be converted into a JSON string</param>
  /// <param name="options">JSON Serializer options</param>
  public override void Write(Utf8JsonWriter writer, TagFilters value, JsonSerializerOptions options)
  {
